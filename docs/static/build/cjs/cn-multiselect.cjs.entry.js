...
const CnMultiselect = class {
    ...
    */
    ...
    render() {
        ...
        return (index.h("div", { key: '1b969bad8c907451919d15412a96d8a059d13c66', class: css_class }, this.popup
            ? index.h("div", { class: this.open ? 'overlay show' : 'overlay' })
            : '', index.h("div", { key: 'ae5ca5acc290e898f2e9ed1b1c9b60a9d1893288', class: "cn-selected-options", onClick: () => this.toggleOptions(), onDrop: event => this.drop(event), onDragOver: event => this.allowDrop(event), onDragEnd: event => this.dragEnd(event) }, selected), index.h("div", { key: 'd8e74967c1d80344d846bd2354b0f77588087cf6', class: "container-options" }, index.h("div", { key: '7a3f929a884a737d2ec353833ce76c4e0fd80587', class: open_options }, this.popup
            ? index.h("div", { class: "popup-header" }, index.h("span", { class: "title" }, this.popupTitle), index.h("span", { class: "close", onClick: () => this.toggleOptions() }))
            : '', index.h("div", { key: '2ff0b99675a9fa8b72f000b7a71b7ff8e4ab6b20', class: "content-options" }, search, index.h("div", { key: 'da861539e268a34b3d5c27d046f05058f8c87306', class: "list-options" }, index.h("div", { key: '13a405eec284aadf35fe68d7b1b1822f3470e50e', class: this.search ? 'options-list' : 'options-list no-search' }, options))), (this.selectAll && this.multiple) ?
            index.h("div", { class: "bulks" }, index.h("button", { onClick: () => this.selectAllItems() }, this.selectAllText), index.h("button", { onClick: () => this.deselectAllItems() }, this.deselectAllText))
            : ''))));
    }
            ...
            if (this.preserveOrder) {
                return this._options.map(option => {
                    if (this._selected.includes(option.value)) {
                        ...
                        return index.h("div", { class: "cn-selected-option", style: style, "data-key": option.value, draggable: true, onDragStart: event => this.drag(event), onClick: event => this.clickItem(option, event) }, index.h("span", null, option.label), index.h("span", { class: "cn-btn-delete-option", onClick: event => this.deselected(option, event) }));
                    }
                });
            }
            else {
                return this._selected.map(option => {
                    ...
                    if (selected) {
                        ...
                        return index.h("div", { class: "cn-selected-option", style: style, "data-key": selected.value, draggable: true, onDragStart: event => this.drag(event), onClick: event => this.clickItem(selected, event) }, index.h("span", null, selected.label), index.h("span", { class: "cn-btn-delete-option", onClick: event => this.deselected(selected, event) }));
                    }
                });
            }
        }
        return index.h("div", { class: "cn-no-values" }, this.title);
    }
    getOptions() {
        if (this._options && this._options.length > 0) {
            const filteredOptions = this._options.map(option => {
                if (this.search && this.searchKey && this.searchKey.length > 2 && !option.label.toLowerCase().match(new RegExp(this.searchKey.toLowerCase()))) {
                    ...
                    return index.h("div", { class: disabled ? "cn-option disabled" : "cn-option" }, index.h("label", { class: "pure-material-checkbox" }, checked
                        ? index.h("input", { type: "checkbox", checked: true, onChange: event => this.toggle(option, event) })
                        : index.h("input", { type: "checkbox", onChange: event => this.toggle(option, event) }), index.h("span", null, index.h("div", null, option.label))));
                }
            });
    ...
    getSearch() {
        return index.h("div", { class: "search" }, index.h("span", null, index.h("input", { placeholder: this.placeholderSearch, ref: (el) => this.searchInput = el, onKeyUp: () => this.searchHandler() }), index.h("div", { class: (this.searchKey && this.searchKey.length > 0) ? 'search-delete active' : 'search-delete', onClick: () => this.clearSearch() })));
    }
    drop(event) {
...
CnMultiselect.style = CnMultiselectStyle0;

exports.cn_multiselect = CnMultiselect;
